...
export default function Chart(p) {
  ...
  chart.update = function() {
    ...
    if (p.grid) {
      ...
      const w = p.margin.left + p.legend.left + (p.labelX.length * p.gridWidth) + p.legend.right + p.margin.right;
      const h = p.margin.top + p.legend.top + (p.labelY.length * p.gridHeight) + p.legend.bottom + p.margin.bottom;
